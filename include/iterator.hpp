...
public:
    ...
    constexpr iterator(const node_pointer& p)
        : ptr_(p)
    {
    }
    ...
    constexpr iterator(const iterator<container_type, iterator_tag, not is_const>& it)
            : ptr_(const_cast<node_pointer>(it.node()))
            {
    }
    ...
    iterator& operator=(const iterator<container_type, iterator_tag, not is_const>& p)
    {
        ptr_ = const_cast<node_pointer>(p.node());
        return *this;
    }
    ...
    typename enable_if<
        is_same<U, forward_iterator_tag>::value or
        is_same<U, bidirectional_iterator_tag>::value or
    ...
    bool operator==(const iterator<container_type, iterator_tag, not is_const>& i) const
    {
    ...
    friend struct iterator<container_type, iterator_tag, not is_const>;
