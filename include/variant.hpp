...
struct type
{
    ...
    constexpr explicit type(const T& val, char* data)
        : ptr_(new(data) T(val))
    {
    }
...
template <typename ...Types>
struct variant final : detail::type<Types>...
{
    constexpr variant()
        : data_()
    {
    ...
    template <typename T>
    constexpr explicit variant(const T& v)
        : detail::type<T>(v, data_)
    {
    }
