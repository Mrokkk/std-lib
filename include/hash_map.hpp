...
private:
    void add_to_bucket(const unsigned hash, const node& kv)
    {
    ...
    typename list<node>::iterator find_in_bucket(const unsigned hash, const Key& key) const
    {
        if (buckets_[hash] == nullptr)
        {
            return nullptr;
    ...
    public:
        ...
        {
            return bucket_index_ == it.bucket_index_ and bucket_iterator_ == it.bucket_iterator_;
        }
    ...
    private:
        detail_iterator(list_array_ptr ptr, const unsigned bucket_index)
            : bucket_ptr_(ptr)
        ...
        detail_iterator(const unsigned bucket_index)
            : bucket_index_(bucket_index)
        {
        }
        ...
        void advance()
        {
            ...
            {
                bucket_index_++;
                while (bucket_ptr_[bucket_index_] == nullptr and bucket_index_ <= Size)
                {
                    bucket_index_++;
                }
                if (bucket_index_ == Size)
